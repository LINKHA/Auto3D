...
TextureMap::TextureMap(char* imagePath)
	: useStencil(false)
	, useDepth(true)
	, useBlend(false)
	, useParallax(false)
...
TextureMap::TextureMap(char* imagePath, const Shader & shader)
	: m_shader(shader)
	, useStencil(false)
	, useDepth(true)
	, useBlend(false)
...
void TextureMap::GenerateMipmap()
{
	glGenerateMipmap(GL_TEXTURE_2D);
	is_Mipmaps = true;
}

void TextureMap::SetColor(const Color& color)
{
	m_Color.Set(color.r, color.g, color.b, color.a);
}

void TextureMap::SetColor(const Vector3& vec)
{
	m_Color.Set(vec.x, vec.y, vec.z, 1.0f);
}
void TextureMap::SetColor(float r, float g, float b, float a)
{
	m_Color.Set(r, g, b, a);
}

void TextureMap::StencilOp(GLenum sfail, GLenum dpfail, GLenum dppass)
{
	m_sfail = sfail;
	m_dpfail = dpfail;
	m_dppass = dppass;
}
void TextureMap::StencilFunc(GLenum func, GLint ref, GLuint mask)
{
	m_func = func;
	m_ref = ref;
	m_mask = mask;
}
void TextureMap::StencilMask(GLuint mask)
{
	m_mas = mask;
}
void TextureMap::DepthFunc(GLenum func)
{
	m_depthfunc = func;
}

//Private
void TextureMap::renderQuad()
{
	if (t_VAO == 0)
	{
		// positions
