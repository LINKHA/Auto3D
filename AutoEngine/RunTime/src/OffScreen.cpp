	...
	, _isAllowLateEffect(false)
{
	...
	_shader = Shader(shader_path + "au_offscreen.auvs"
		, shader_path + "au_offscreen_test.aufs");
...
void OffScreen::bindMsaaAndPostpro()
{
	...
	glGenTextures(1, &_hdr);
	glBindTexture(GL_TEXTURE_2D, _hdr);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, _screenTexture, 0);	// we only need a color buffer
