	...
	: tTexture3D(ambient)
{
	...
	Singleton<SkyBoxManager>::Instance().AddSkyBox(this);
}
...
{
	UnloadOpaque(SharedFromThis(HDRSkyBox));
}
...
void HDRSkyBox::Start()
{
	...
	hdrTexture = GetSubSystem<ResourceSystem>()->HdrLoad((dir + "texture/hdr/Mt-Washington-Gold-Room_Ref.hdr").CStr());
	...
	RegisterOpaque(SharedFromThis(HDRSkyBox));
}
void HDRSkyBox::Draw()
{
	glDepthFunc(GL_LEQUAL);
