...
_Shader::_Shader(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
...
// ------------------------------------------------------------------------
void _Shader::SetVec2(const STRING &name, const glm::vec2 &value) const
{
	glUniform2fv(glGetUniformLocation(ID, name.CStr()), 1, &value[0]);
}
void _Shader::SetVec2(const STRING &name, const Vector2 &value) const
{
	glUniform2f(glGetUniformLocation(ID, name.CStr()), value.x, value.y);
}
void _Shader::SetVec2(const STRING &name, float x, float y) const
{
	glUniform2f(glGetUniformLocation(ID, name.CStr()), x, y);
}
// ------------------------------------------------------------------------
void _Shader::SetVec3(const STRING &name, const glm::vec3 &value) const
{
	glUniform3fv(glGetUniformLocation(ID, name.CStr()), 1, &value[0]);
}
void _Shader::SetVec3(const STRING &name, const Color &value) const
{
	glUniform3f(glGetUniformLocation(ID, name.CStr()), value.r, value.g, value.b);
}
void _Shader::SetVec3(const STRING &name, const Vector3 &value) const
{
	glUniform3f(glGetUniformLocation(ID, name.CStr()), value.x, value.y, value.z);
}
void _Shader::SetVec3(const STRING &name, float x, float y, float z) const
{
	glUniform3f(glGetUniformLocation(ID, name.CStr()), x, y, z);
}
// ------------------------------------------------------------------------
void _Shader::SetVec4(const STRING &name, const glm::vec4 &value) const
{
	glUniform4fv(glGetUniformLocation(ID, name.CStr()), 1, &value[0]);
}
void _Shader::SetVec4(const STRING &name, const Vector4 &value) const
{
	glUniform4f(glGetUniformLocation(ID, name.CStr()), value.x, value.y, value.z, value.w);
}
void _Shader::SetVec4(const STRING &name, float x, float y, float z, float w)
{
	glUniform4f(glGetUniformLocation(ID, name.CStr()), x, y, z, w);
}
// ------------------------------------------------------------------------
void _Shader::SetMat2(const STRING &name, const glm::mat2 &mat) const
{
	glUniformMatrix2fv(glGetUniformLocation(ID, name.CStr()), 1, GL_FALSE, &mat[0][0]);
}
// ------------------------------------------------------------------------
void _Shader::SetMat3(const STRING &name, const glm::mat3 &mat) const
{
	glUniformMatrix3fv(glGetUniformLocation(ID, name.CStr()), 1, GL_FALSE, &mat[0][0]);
}
// ------------------------------------------------------------------------
void _Shader::SetMat4(const STRING &name, const glm::mat4 &mat) const
{
	glUniformMatrix4fv(glGetUniformLocation(ID, name.CStr()), 1, GL_FALSE, &mat[0][0]);
}
void _Shader::checkCompileErrors(GLuint shader, STRING type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
