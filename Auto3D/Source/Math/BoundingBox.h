...
public:
	...
	BoundingBox Transformed(const Matrix3x3<_Ty>& transform) const;
...
template<typename _Ty> void BoundingBox<_Ty>::Define(const Vector3<_Ty>* vertices, size_t count)
{
	Undefine();
	Merge(vertices, count);
}

template<typename _Ty> void BoundingBox<_Ty>::Define(const Frustum& frustum)
{
	Define(frustum._vertices, NUM_FRUSTUM_VERTICES);
}

template<typename _Ty> void BoundingBox<_Ty>::Define(const Polyhedron& poly)
{
	Undefine();
	Merge(poly);
}

template<typename _Ty> void BoundingBox<_Ty>::Define(const Sphere& sphere)
{
...
template<typename _Ty> void BoundingBox<_Ty>::Merge(const Vector3<_Ty>* vertices, size_t count)
{
	while (count--)
		Merge(*vertices++);
}

template<typename _Ty> void BoundingBox<_Ty>::Merge(const Frustum& frustum)
{
	Merge(frustum._vertices, NUM_FRUSTUM_VERTICES);
}

template<typename _Ty> void BoundingBox<_Ty>::Merge(const Polyhedron& poly)
{
...
template<typename _Ty> void BoundingBox<_Ty>::Merge(const Sphere& sphere)
{
	const Vector3<_Ty>& center = sphere._center;
	_Ty radius = sphere._radius;
...
template<typename _Ty> void BoundingBox<_Ty>::Clip(const BoundingBox<_Ty>& box)
{
	if (box._min._x > _min._x)
		_min._x = box._min._x;
	if (box._max._x < _max._x)
...
template<typename _Ty> void BoundingBox<_Ty>::Transform(const Matrix3x3<_Ty>& transform)
{
	*this = Transformed(Matrix3x4<_Ty>(transform));
}

template<typename _Ty> void BoundingBox<_Ty>::Transform(const Matrix3x4<_Ty>& transform)
{
	*this = Transformed(transform);
}

template<typename _Ty> bool BoundingBox<_Ty>::FromString(const String& str)
{
	return FromString(str.CString());
}

template<typename _Ty> bool BoundingBox<_Ty>::FromString(const char* str)
{
...
template<typename _Ty> BoundingBox<_Ty> BoundingBox<_Ty>::Transformed(const Matrix3x3<_Ty>& transform) const
{
	return Transformed(Matrix3x4<_Ty>(transform));
}

template<typename _Ty> BoundingBox<_Ty> BoundingBox<_Ty>::Transformed(const Matrix3x4<_Ty>& transform) const
{
...
template<typename _Ty> Rect<_Ty> BoundingBox<_Ty>::Projected(const Matrix4x4<_Ty>& projection) const
{
...
template<typename _Ty> Intersection BoundingBox<_Ty>::IsInside(const Sphere& sphere) const
{
	_Ty distSquared = 0;
	_Ty temp;
	const Vector3<_Ty>& center = sphere._center;
...
template<typename _Ty> Intersection BoundingBox<_Ty>::IsInsideFast(const Sphere& sphere) const
{
	_Ty distSquared = 0;
	_Ty temp;
	const Vector3<_Ty>& center = sphere._center;
...
template<typename _Ty> String BoundingBox<_Ty>::ToString() const
{
