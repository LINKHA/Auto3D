...
public:
	...
	template <class _Ty> _Ty* CreateChild() { return static_cast<_Ty*>(CreateChild(_Ty::TypeStatic())); }
	/// Create named child node of the specified type, template version.
	template <class _Ty> _Ty* CreateChild(const String& childName) { return static_cast<_Ty*>(CreateChild(_Ty::TypeStatic(), childName)); }
	/// Create named child node of the specified type, template version.
	...
	template <class _Ty> _Ty* FindChild(bool recursive = false) const { return static_cast<_Ty*>(FindChild(_Ty::TypeStatic(), recursive)); }
	/// Return first child node that matches type and name, template version.
	template <class _Ty> _Ty* FindChild(const String& childName, bool recursive = false) const { return static_cast<_Ty*>(FindChild(_Ty::TypeStatic(), childName, recursive)); }
	/// Return first child node that matches type and name, template version.
	template <class _Ty> _Ty* FindChild(const char* childName, bool recursive = false) const { return static_cast<_Ty*>(FindChild(_Ty::TypeStatic(), childName, recursive)); }
