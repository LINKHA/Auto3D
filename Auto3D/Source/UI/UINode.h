...
//	/// Return unique _id within the canvas, or 0 if not in a canvas.
...
//	/// Set name. Is not required to be unique within the canvas.
...
//	/// Set node's layer by name. The layer name must have been registered to the canvas root beforehand.
...
//	/// Set node's tag by name. The tag name must have been registered to the canvas root beforehand.
...
//	/// Return layer name, or empty if not registered in the canvas root.
...
//	/// Return tag name, or empty if not registered in the canvas root.
//	const String& GetTagName() const;
//	/// Return parent node.
//	UINode* Parent() const { return _parent; }
//	/// Return the canvas that the node belongs to.
...
//	virtual void OnCanvasSet(Canvas* newCanvas, Canvas* oldCanvas);
