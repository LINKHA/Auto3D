...
void FForwardShadingRenderer::RenderBatches()
{
	...
	for (auto it = batches.Begin(); it != batches.End();)
	{
		...
		for (uint8_t ii = 0; ii < drawNum; ++ii)
		{
			...
			RenderState renderStates = { // ShadowMap_PackDepth
				0
				| BGFX_STATE_WRITE_RGB
				| BGFX_STATE_WRITE_A
				| BGFX_STATE_WRITE_Z
		...
		// Draw scene.
		{
			...
			{
				bx::mtxMul(FShadowRenderer::s_lightMtx, modelMatrix.Data(), mtxShadow);
			}

			RenderState renderStates2 = { // ShadowMap_PackDepth
				0
				| BGFX_STATE_WRITE_RGB
				| BGFX_STATE_WRITE_A
				| BGFX_STATE_DEPTH_TEST_LEQUAL
			...
			Submit2(geometry, RENDERVIEW_DRAWSCENE_0_ID
				,modelMatrix.Data()
