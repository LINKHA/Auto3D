...
void FApplication::Run()
{
	...
	//if (_engine.get())
	//	_engine = std::make_unique<AEngine>();
...
static PlatfromContext platfromContext;

const Event* poll()
{
	return platfromContext.m_eventQueue.poll();
}

const Event* poll(WindowHandle _handle)
{
	return platfromContext.m_eventQueue.poll(_handle);
}

void release(const Event* _event)
{
	platfromContext.m_eventQueue.release(_event);
}

WindowHandle createWindow(int32_t _x, int32_t _y, uint32_t _width, uint32_t _height, uint32_t _flags, const char* _title)
{
	bx::MutexScope scope(platfromContext.m_lock);
	WindowHandle handle = { platfromContext.m_windowAlloc.alloc() };
...
void destroyWindow(WindowHandle _handle)
{
	if (UINT16_MAX != _handle.idx)
	{
		...
		bx::MutexScope scope(platfromContext.m_lock);
		platfromContext.m_windowAlloc.free(_handle.idx);
	}
}
