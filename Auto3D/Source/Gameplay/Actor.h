...
public:
    ...
    ///// Create named child node of specified type.
    //AActor* CreateChildNode(FStringHash childType, const char* childName);
    ...
    //AActor* FindChildNodeByIndex(size_t index) const { return index < _childrenNode.Size() ? _childrenNode[index].get() : nullptr; }
    /// Return all immediate child nodes.
    const TVector<AActor*>& GetChildrenNode() const { return _childrenNode; }
    /// Return child nodes recursively.
    void GetAllChildrenNode(TVector<AActor*>& result) const;
    /// Return first child node that matches name.
    AActor* FindChildNode(const FString& childName, bool recursive = false) const;
    /// Return first child node that matches name.
    AActor* FindChildNode(const char* childName, bool recursive = false) const;
    /// Return first child node of specified type.
    AActor* FindChildNode(FStringHash childType, bool recursive = false) const;
    /// Return first child node that matches type and name.
    AActor* FindChildNode(FStringHash childType, const FString& childName, bool recursive = false) const;
    /// Return first child node that matches type and name.
    AActor* FindChildNode(FStringHash childType, const char* childName, bool recursive = false) const;
    ...
    template <typename _Ty> void FindChildrenNode(TVector<_Ty*>& result, bool recursive = false) const { return FindChildrenNode(reinterpret_cast<TVector<AActor*>&>(result), _Ty::GetTypeHashStatic(), recursive); }
	...
	ATransform* GetTransform() { return _transform; }
protected:
