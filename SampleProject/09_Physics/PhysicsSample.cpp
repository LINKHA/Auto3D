...
//
//		//btCollisionShape* colShape = new btBoxShape(btVector3(1,1,1));
//		btBoxShape* colShape = new btBoxShape(btVector3(1.0f, 1.0f, 1.0f));
//		collisionShapes.push_back(colShape);
//
//		/// Create Dynamic Objects
//		btTransform startTransform;
...
//			colShape->calculateLocalInertia(mass, localInertia);
//
//		startTransform.setOrigin(btVector3(2.6, 5, 0));
//
//		//using motionstate is recommended, it provides interpolation capabilities, and only synchronizes 'active' objects
//		btDefaultMotionState* myMotionState = new btDefaultMotionState(startTransform);
//		btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, myMotionState, colShape, localInertia);
...
//
//	box3 = scene->CreateChild<StaticModel>();
//	box3->SetScale(Vector3F(1.0f, 1.0f, 1.0f));
//	box3->SetModel(cache->LoadResource<Model>("Box.mdl"));
//}
...
//		if (j == 1)
//		{
//			box2->SetPosition(Vector3F(float(trans.getOrigin().getX()), float(trans.getOrigin().getY()), float(trans.getOrigin().getZ())));
//			//box2->SetRotation(Quaternion(ToQuaternion(trans.getRotation())));
//		}
//		if (j == 2)
//		{
//			box3->SetPosition(Vector3F(float(trans.getOrigin().getX()), float(trans.getOrigin().getY())+0.5f, float(trans.getOrigin().getZ())));
//			//box2->SetRotation(Quaternion(ToQuaternion(trans.getRotation())));
//		}
