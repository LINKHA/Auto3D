...
float DistributionGGX(vec3 N, vec3 H, float roughness)
{
    ...
    float NdotH2 = mul(NdotH, NdotH);

    float nom   = a2;
    float denom = (mul(NdotH2,(a2 - 1.0)) + 1.0);
    denom = mul(mul(PI, denom), denom);
...
vec3 ImportanceSampleGGX(vec2 Xi, vec3 N, float roughness)
{
	...
	float a = mul(roughness, roughness);
	
	float phi = mul(mul(2.0, PI), Xi.x);
	float cosTheta = sqrt((1.0 - Xi.y) / (1.0 + mul((mul(a, a) - 1.0), Xi.y)));
	float sinTheta = sqrt(1.0 - cosTheta*cosTheta);
	
	// from spherical coordinates to cartesian coordinates - halfway vector
	vec3 H;
	H.x = mul(cos(phi), sinTheta);
	H.y = mul(sin(phi), sinTheta);
	H.z = cosTheta;
	...
	vec3 sampleVec =  mul(tangent, H.x) +  mul(bitangent, H.y) +  mul(N, H.z);
