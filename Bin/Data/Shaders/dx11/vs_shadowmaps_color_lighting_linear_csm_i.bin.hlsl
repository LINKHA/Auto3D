...
// shaderc -f D:\Auto3D\SampleProject\16-shadowmaps\vs_shadowmaps_color_lighting_linear_csm_i.sc -o D:\Auto3D\Bin\Data\Shaders\dx11\vs_shadowmaps_color_lighting_linear_csm_i.bin --depends -i D:\Auto3D\Auto3D\ThirdParty\bgfx\bgfx\src --varyingdef D:\Auto3D\SampleProject\16-shadowmaps\varying.def.sc --platform windows -p vs_4_0 -O 3 --type vertex -O3 --debug -O 0

struct Output
{
float4 gl_Position : SV_POSITION;
float3 v_normal : NORMAL;
float4 v_position : TEXCOORD7;
float4 v_texcoord1 : TEXCOORD1;
float4 v_texcoord2 : TEXCOORD2;
float4 v_texcoord3 : TEXCOORD3;
float4 v_texcoord4 : TEXCOORD4;
...
static float4x4 u_lightMtx;
uniform float4x4 u_shadowMapMtx0;
uniform float4x4 u_shadowMapMtx1;
uniform float4x4 u_shadowMapMtx2;
uniform float4x4 u_shadowMapMtx3;
Output main( float4 a_normal : NORMAL , float3 a_position : POSITION) { Output _varying_; _varying_.v_normal = float3(0.0, 0.0, 1.0);; _varying_.v_position = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_texcoord1 = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_texcoord2 = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_texcoord3 = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_texcoord4 = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_view = float3(0.0, 0.0, 0.0);;
{
...
float4 posOffset = float4(a_position + normal.xyz * u_params1.y, 1.0);
_varying_.v_position = mul(u_modelView, posOffset);
float4 wpos = float4(mul(u_model[0], posOffset).xyz, 1.0);
_varying_.v_texcoord1 = mul(u_shadowMapMtx0, wpos);
_varying_.v_texcoord2 = mul(u_shadowMapMtx1, wpos);
_varying_.v_texcoord3 = mul(u_shadowMapMtx2, wpos);
_varying_.v_texcoord4 = mul(u_shadowMapMtx3, wpos);
_varying_.v_texcoord1.z += 0.5;
_varying_.v_texcoord2.z += 0.5;
_varying_.v_texcoord3.z += 0.5;
_varying_.v_texcoord4.z += 0.5;
} return _varying_;
}
