...
// shaderc -f D:\Project\MyProject\Auto3D\SampleProject\15-shadowmaps-simple\vs_sms_mesh_i.sc -o D:\Project\MyProject\Auto3D\Bin\Data\Shaders\dx11\vs_sms_mesh_i.bin --depends -i D:\Project\MyProject\Auto3D\Auto3D\ThirdParty\bgfx\bgfx\src --varyingdef D:\Project\MyProject\Auto3D\SampleProject\15-shadowmaps-simple\varying.def.sc --platform windows -p vs_4_0 -O 3 --type vertex -O3 --debug -O 0

struct Output
{
float4 gl_Position : SV_POSITION;
float3 v_normal : NORMAL;
float4 v_shadowcoord : TEXCOORD1;
float3 v_view : TEXCOORD0;
};
...
uniform float4x4 u_shadowMtx;
Output main( float4 a_normal : NORMAL , float3 a_position : POSITION , float4 i_data0 : TEXCOORD7 , float4 i_data1 : TEXCOORD6 , float4 i_data2 : TEXCOORD5 , float4 i_data3 : TEXCOORD4) { Output _varying_; _varying_.v_normal = float3(0.0, 0.0, 1.0);; _varying_.v_shadowcoord = float4(0.0, 0.0, 0.0, 0.0);; _varying_.v_view = float3(0.0, 0.0, 0.0);;
{
float4x4 model;
model[0] = i_data0;
model[1] = i_data1;
model[2] = i_data2;
model[3] = i_data3;
float4x4 lightMtx = mul(u_shadowMtx, model);
float4 worldPos = instMul(model, float4(a_position, 1.0) );
_varying_.gl_Position = mul(u_viewProj, worldPos );
...
float3 posOffset = a_position + normal.xyz * shadowMapOffset;
_varying_.v_shadowcoord = mul(lightMtx, float4(posOffset, 1.0) );
} return _varying_;
}
